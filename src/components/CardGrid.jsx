...
const CardGrid = () =>
{
    ...
    return (
        <> <div className='flex justify-center'> <div className='max-w-7xl  ' style={{     padding: '40px 20px', fontFamily: "'Georgia', serif" }}>
        ...
        <div className="pb-25   flex justify-center items-center font-mono"
            style={{    fontFamily: "'Georgia', serif" }}
        >
            
            <div className="container w-[90%] flex flex-wrap justify-between gap-5 max-[1024px]:justify-center">
                {cards.map((card, index) => (
                    <div
                        key={index}
                        className={`relative w-[440px] h-[400px] overflow-hidden rounded-lg cursor-pointer group`}
    ...
    ${activeCard === index ? 'translate-y-0' : 'translate-y-full group-hover:translate-y-0 max-[768px]:group-hover:translate-y-full'}`}
                        >
                            ...
                            <div className="flex-1 flex flex-col justify-center items-start w-full text-left space-y-3">
                                <h3 className='text-center  w-full text-2xl pb-2 bg-gray-200 rounded-2xl'>{card.title}</h3>
